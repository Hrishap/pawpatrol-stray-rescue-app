...
import { claimCase, useCases } from '@/hooks/useCases';
...
export function MapScreen({ variant }: MapScreenProps) {
  ...
  const claimNearest = async () => {
    const open = cases.filter((c) => c.status === 'open');
    if (open.length === 0) {
      Alert.alert(t('toastNoUnclaimedNearby'));
      return;
    }
    const nearest = open.reduce((best, c) => {
      const d = haversineKm(coords, { lat: c.lat, lng: c.lng });
      const bestD = haversineKm(coords, { lat: best.lat, lng: best.lng });
      return d < bestD ? c : best;
    });
    setClaiming(true);
    try {
      await claimCase(nearest.id);
      Alert.alert(t('toastClaimedNearest', { code: nearest.code }));
      router.push(`/case/${nearest.id}`);
    } catch (err) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.mapWrap}>
          ...
          <View style={styles.popup}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.popupMeta}>
                {selectedCase.address ?? formatDistance(haversineKm(coords, selectedCase))}
              </Text>
            </View>
            <Pressable
        ...
        <Pressable
          style={styles.fab}
          disabled={claiming}
          onPress={() => (variant === 'volunteer' ? claimNearest() : router.push('/report/step1'))}
        >
          <Text style={variant === 'volunteer' ? styles.fabCheck : styles.fabPlus}>
            {variant === 'volunteer' ? '✓' : '+'}
          </Text>
        </Pressable>
      </View>
...
const styles = StyleSheet.create({
  ...
  fabCheck: { fontSize: 24, color: colors.background },
});
